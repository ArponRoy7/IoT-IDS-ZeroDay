...
# Dual Residual (Magnitude + Temporal Variance)
...
for ZERO_DAY in ZERO_DAY_LIST:
    ...
    # =====================================================
    # RF TRAIN — DUAL RESIDUAL FEATURE
    # =====================================================
    ...
    # ---- Temporal Variance Feature ----
    residual_series = pd.Series(residual_rf)
    variance_rf = residual_series.rolling(window=10, min_periods=1).var().fillna(0).values
    ...
    print("RF trained with Dual-Residual Features")
    ...
    # ---- Apply same variance feature ----
    eval_series = pd.Series(residual_eval)
    variance_eval = eval_series.rolling(window=10, min_periods=1).var().fillna(0).values
    ...
    drift_df.to_csv(f"concept_drift_{ZERO_DAY}.csv", index=False)
    print(f"Concept Drift Log saved as concept_drift_{ZERO_DAY}.csv")

    print("Benign Recall:",
          round(recall_score(y_eval=="BENIGN",
                             hybrid_preds=="BENIGN"),4))

    print("Zero-Day Recall:",
          round(recall_score(y_eval==ZERO_DAY,
                             hybrid_preds=="ZERO_DAY"),4))

print("\nDual-Residual Adaptive Hybrid Completed")