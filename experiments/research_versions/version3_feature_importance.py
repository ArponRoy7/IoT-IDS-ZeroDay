...
# INCLUDES EXPLAINABILITY (XAI) FEATURE IMPORTANCE
# =========================================================
...
# 🔥 XAI: Store the original feature names
original_feature_names = list(df.drop("Label", axis=1).columns)
# Append the augmented hybrid features
augmented_feature_names = original_feature_names + ["DAE_Residual", "DAE_Variance"]
...
for ZERO_DAY in ZERO_DAY_LIST:
    ...
    # 🔥 XAI: EXTRACT AND PRINT FEATURE IMPORTANCES
    importances = rf.feature_importances_
    indices = np.argsort(importances)[::-1]
    
    print(f"\n--- EXPLAINABILITY: TOP 10 FEATURES FOR {ZERO_DAY} ---")
    for f in range(10):
        feature_name = augmented_feature_names[indices[f]]
        importance_score = importances[indices[f]]
        print(f"{f + 1}. {feature_name} (Score: {importance_score:.4f})")
    print("----------------------------------------------------\n")
